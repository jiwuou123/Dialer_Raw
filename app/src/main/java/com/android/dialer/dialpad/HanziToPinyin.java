 ...
 * with ICU.
 *
 * Currently this file is aligned to zh.txt
 */
public class HanziToPinyin {
    ...
    public boolean isChinese(String s){
        return COLLATOR.compare(s,FIRST_PINYIN_UNIHAN) >= 0&&COLLATOR.compare(s,LAST_PINYIN_UNIHAN) <= 0;
    }
    public boolean isChinese(char character){
        return isChinese(Character.toString(character));
    }
    ...
    public Token getAcronymToken(char character) {
        Token token = new Token();
        final String letter = Character.toString(character);
        token.source = letter;
        int offset = -1;
        ...
        StringBuilder pinyin = new StringBuilder();
        for (int j = 0; j < 1 && PINYINS[offset][j] != 0; j++) {
            pinyin.append((char) PINYINS[offset][j]);
        }
        token.target = pinyin.toString().toLowerCase();
        if (TextUtils.isEmpty(token.target)) {
            token.type = Token.UNKNOWN;
            token.target = token.source;
        }
    ...
    public ArrayList<Token> getAcronym(final String input) {
        ...
        for (int i = 0; i < inputLength; i++) {
            ...
            if (character == ' ') {
                ...
                Token t = getAcronymToken(character);
